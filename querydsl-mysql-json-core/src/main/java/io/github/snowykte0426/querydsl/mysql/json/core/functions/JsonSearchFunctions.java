 ...
 *
 * <p>This class provides static factory methods for searching within JSON documents:
 * <ul>
...
public final class JsonSearchFunctions {
     ...
     * @param path the JSON path (e.g., "$.key")
     ...
     *
     * <p>This is equivalent to {@code JSON_UNQUOTE(JSON_EXTRACT(json_doc, path))}</p>
     *
     * @param jsonDoc the JSON document expression
     ...
     *
     * <p>Available in MySQL 8.0.21+. This function extracts a single scalar value
     * and returns NULL for non-scalar values.</p>
     *
     * @param jsonDoc the JSON document expression
     * @param path the JSON path
     ...
     *
     * @param jsonDoc the JSON document expression
     * @param value the value to search for (must be valid JSON)
     ...
     * @param path the JSON path to search within
     * @return boolean expression
     */
     ...
     *
     * @param jsonDoc the JSON document expression
     * @param value the value expression to search for
     * @return boolean expression
     */
     ...
     *
     * <p>SQL: {@code JSON_CONTAINS_PATH(json_doc, one_or_all, path, ...)}</p>
     *
     * <p>Example:
     * <pre>{@code
    ...
    public static JsonArrayExpression jsonKeys(Expression<?> jsonDoc) {
        return JsonArrayExpression.wrap(
            Expressions.stringTemplate("json_keys({0})", jsonDoc)
        );
    }
     ...
     *
     * <p>SQL: {@code JSON_SEARCH(json_doc, 'one', search_str)}</p>
     *
     * <p>The search string supports wildcards % and _.</p>
     *
     * @param jsonDoc the JSON document expression
     ...
     * @return path(s) to the value
     */
    public static JsonValueExpression jsonSearch(
        Expression<?> jsonDoc,
        String oneOrAll,
        String searchString,
        String escapeChar,
    ...
    /**
     * Tests whether two JSON documents have any key-value pairs or array elements in common.
     *
     * <p>SQL: {@code JSON_OVERLAPS(json_doc1, json_doc2)}</p>
     *
     * <p>Available in MySQL 8.0.17+</p>
     *
     ...
     *
     * <p>SQL: {@code value MEMBER OF(json_array)}</p>
     *
     * <p>Available in MySQL 8.0.17+</p>
     *
     * <p>Example:
     ...
     *
     * @param valueExpr the value expression to test
     * @param jsonArray the JSON array expression
     * @return boolean expression
     */
