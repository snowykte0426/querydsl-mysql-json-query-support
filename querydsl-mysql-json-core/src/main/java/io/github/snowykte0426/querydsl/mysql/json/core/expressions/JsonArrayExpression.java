 ...
 *
 * <p>This class provides specialized operations for JSON arrays,
 * including array manipulation, element access, and array-specific functions.</p>
 *
 * <p>Example usage:
 * <pre>{@code
 * // Create a JSON array
...
public class JsonArrayExpression extends JsonExpression<String> {
    ...
    public static JsonArrayExpression empty() {
        return new JsonArrayExpression(
            Expressions.stringTemplate("json_array()")
        );
    }
     ...
     *
     * @param path the JSON path where to append
     ...
     *
     * @param path the JSON path where to insert (e.g., "$[0]")
     ...
     *
     * <p>SQL: {@code JSON_ARRAYAGG(col_or_expr)}</p>
     *
    ...
    public static JsonArrayExpression aggregate(Expression<?> expression) {
        return new JsonArrayExpression(
            Expressions.stringTemplate("json_arrayagg({0})", expression)
        );
    }
