 ...
 * {@code META-INF/services/org.hibernate.boot.model.FunctionContributor} SPI file.
 * No manual configuration is required - Hibernate will automatically load this
 * contributor when it's on the classpath.
 * <p>
 * This contributor registers all MySQL JSON functions with their correct return types,
 * enabling them to be used in HQL/JPQL queries without "Non-boolean expression used in
 * predicate context" errors.
 * <p>
 * <strong>Functions Registered:</strong>
 * <ul>
 *     <li>Boolean functions: json_contains, json_contains_path, json_overlaps, json_valid, etc.</li>
 *     <li>String functions: json_extract, json_unquote, json_type, json_search, etc.</li>
...
public class MySQLJsonFunctionContributor implements FunctionContributor {
     ...
     * <p>
     * These functions can be used directly in WHERE clauses without additional type conversions.
     *
     * @param fc the function contributions registry
     * @param typeConfig the type configuration for resolving return types
     */
    private void registerBooleanFunctions(FunctionContributions fc, TypeConfiguration typeConfig) {
        ...
        // json_contains_path with multiple paths
        fc.getFunctionRegistry().registerPattern(
            "json_contains_path_multi",
            "json_contains_path(?1, ?2, ?3, ?4)",
            typeConfig.getBasicTypeRegistry().resolve(StandardBasicTypes.BOOLEAN)
        );

        // json_overlaps(json1, json2) - Returns 1 if json1 and json2 have any overlapping elements
        fc.getFunctionRegistry().registerPattern(
            "json_overlaps",
            "json_overlaps(?1, ?2)",
            typeConfig.getBasicTypeRegistry().resolve(StandardBasicTypes.BOOLEAN)
        ...
        // json_schema_valid(schema, document) - Returns 1 if document validates against schema
        fc.getFunctionRegistry().registerPattern(
            "json_schema_valid",
    ...
    private void registerStringFunctions(FunctionContributions fc, TypeConfiguration typeConfig) {
        ...
        // json_search(json_doc, one_or_all, search_str) - Searches for string in JSON document
        fc.getFunctionRegistry().registerPattern(
            "json_search",
            "json_search(?1, ?2, ?3)",
            typeConfig.getBasicTypeRegistry().resolve(StandardBasicTypes.STRING)
        ...
        // json_search with escape character
        fc.getFunctionRegistry().registerPattern(
            "json_search_escape",
            "json_search(?1, ?2, ?3, ?4)",
            typeConfig.getBasicTypeRegistry().resolve(StandardBasicTypes.STRING)
        ...
        // json_search with escape and path
        fc.getFunctionRegistry().registerPattern(
            "json_search_path",
    ...
    private void registerNumericFunctions(FunctionContributions fc, TypeConfiguration typeConfig) {
        ...
        // json_storage_free(json_doc) - Returns freed space in bytes after partial update
        fc.getFunctionRegistry().registerPattern(
            "json_storage_free",
            "json_storage_free(?1)",
            typeConfig.getBasicTypeRegistry().resolve(StandardBasicTypes.INTEGER)
